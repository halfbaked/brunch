(function() {
  var colors, expressProcess, fakeServerPackage, fileUtil, fs, helpers, package, path, root, spawn, stitch, timeouts, _;
  root = __dirname + "/../";
  fs = require('fs');
  path = require('path');
  ...
  exports.build = function(options) {
    ...
    return exports.buildFakeServer();
  };
  ...
  exports.compilePackage = function() {
    return package.compile(function(err, source) {
      if (err != null) {
        ...
        return fs.writeFile(path.join(exports.options.buildPath, 'spa/js/app.js'), source, function(err) {
          if (err != null) {
  ...
  exports.buildFakeServer = function() {
    var executeCoffee;
    executeCoffee = spawn('coffee', ['-o', path.join(exports.options.buildPath, 'fake_server'), '-c', path.join(exports.options.brunchPath, 'src/fake_server')]);
  ...
  exports.spawnStylus = function() {
    ...
    executeStylus = spawn('stylus', ['-I', path.join(exports.options.brunchPath, 'src/vendor/styles'), '--compress', '--out', path.join(exports.options.buildPath, 'spa/css'), path.join(exports.options.brunchPath, 'src/spa/styles/main.styl'), path.join(exports.options.brunchPath, 'src/vendor/styles/')]);
